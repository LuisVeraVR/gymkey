...
export class AccessKeysController {
  ...
  @Get('my-key')
  @Roles(UserRole.MEMBER, UserRole.COACH, UserRole.STAFF, UserRole.GYM_ADMIN, UserRole.SUPER_ADMIN)
  async getMyKey(@Request() req: any) {
    return this.accessKeysService.generateKey(req.user.userId, req.user.tenantId);
  }

  // Endpoint for Admin/Staff to validate the QR Token
  @Post('validate')
  @UseGuards(RolesGuard)
  @Roles(UserRole.STAFF, UserRole.GYM_ADMIN, UserRole.SUPER_ADMIN, UserRole.COACH)
